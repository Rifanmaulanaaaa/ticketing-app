    ...
    <section class="max-w-7xl mx-auto py-12 px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($events as $event)
                <x-user.event-card :title="$event->judul" :date="$event->tanggal_waktu" :location="$event->lokasi?->nama_lokasi ?? 'Lokasi Tidak Ada'"
                    :price="$event->tikets_min_harga" :image="$event->gambar" :href="route('events.show', ['event' => $event->id])" />
            @endforeach
        </div>
    </section>
</x-layouts.app>