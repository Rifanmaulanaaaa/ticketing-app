<x-layouts.app>
  ...
  document.querySelectorAll('input[id^="qty-"]').forEach(input => {
    input.addEventListener('change', e => {
      ...
      if (val < 0) val = 0;
      if (val > tickets[id].stock) val = tickets[id].stock;
  ...
  window.openCheckout = function () {
    ...
    modalItems.innerHTML = html || '<p class="text-gray-500">Belum ada item.</p>';
    modalTotal.textContent = formatRupiah(total);
    modal.showModal();
  };

  // ======================
  // CONFIRM CHECKOUT (AJAX)
  // ======================
